...
def get_opt():
    ...
    parser.add_argument("--name1", default="TOM")
    ...
    parser.add_argument("--datamode1", default="test")
    
    parser.add_argument("--stage", default="GMM")
    parser.add_argument("--stage1", default="TOM")
    ...
    parser.add_argument('--checkpoint1', type=str, default='checkpoints/TOM/tom_final.pth', help='model checkpoint for test')
...
def test_gmm(opt, test_loader, model, board):
    ...
    save_dir = os.path.join(opt.result_dir1, opt.datamode)
...
def test_tom(opt1, test_loader1, model1, board1):
    model1.cuda()
    model1.eval()

    base_name = os.path.basename(opt1.checkpoint1)
    # save_dir = os.path.join(opt.result_dir, base_name, opt.datamode)
    save_dir = os.path.join(opt1.result_dir, opt1.datamode1)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    print('Dataset size: %05d!' % (len(test_loader1.dataset)), flush=True)
    for step, inputs in enumerate(test_loader1.data_loader):
        iter_start_time = time.time()
        ...
        outputs = model1(torch.cat([agnostic, c, cm], 1))  # CP-VTON+
        ...
        if (step+1) % opt1.display_count == 0:
            board_add_images(board1, 'combine', visuals, step+1)
            t = time.time() - iter_start_time
            print('step: %8d, time: %.3f' % (step+1, t), flush=True)
...
def main():
    ...
    opt1 = get_opt()
    #print(opt)
    print("Start to test stage: %s, named: %s!" % (opt.stage1, opt.name1))

    # create dataset
    test_dataset1 = CPDataset(opt1)

    # create dataloader
    test_loader1 = CPDataLoader(opt1, test_dataset1)

    # visualization
    if not os.path.exists(opt.tensorboard_dir):
        os.makedirs(opt.tensorboard_dir)
    board1 = SummaryWriter(logdir=os.path.join(opt.tensorboard_dir, opt.name1))
    
    
    # model = UnetGenerator(25, 4, 6, ngf=64, norm_layer=nn.InstanceNorm2d)  # CP-VTON
    model1 = UnetGenerator(26, 4, 6, ngf=64, norm_layer=nn.InstanceNorm2d)  # CP-VTON+
    load_checkpoint(model1, opt.checkpoint1)
    with torch.no_grad():
        test_tom(opt1, test_loader1, model1, board1)
    
    #else:
    #    raise NotImplementedError('Model [%s] is not implemented' % opt.stage)

    print('Finished test %s, named: %s!' % (opt.stage1, opt.name1))
