...
from matplotlib import pyplot as plt # To create static, animated and interactive visualizations in python.
import sys
import shutil # To do high-level operations on files and collection of files. Operations like copying and removing files.
...
def decode_labels(mask):
    ...
    mask = np.expand_dims(mask, axis=2) # mask.shape will become hXwX1
    h, w, c = mask.shape
    ...
    pixels = par_img.load()
    for j_, j in enumerate(mask[:, :, 0]): # enumerate adds a counter to the iterable, here j_ = row index of mask and j = values in that row
        for k_, k in enumerate(j):# here k_ will have index of the elements of one row indexed by j_ and k will have that value of that row
            if k < N_CLASSES:
                pixels[k_, j_] = label_colours[k] # If the pixel value of mask has values b/w 0 to 20 then copy them to pixels.
...
def body_detection(image, seg_mask):# Doubtful
...
def shape_from_contour(img, contour):
    ...
    dummy_mask = cv2.drawContours(
        dummy_mask, [contour], 0, (1, 0, 0), thickness=cv2.FILLED)# To draw contours according to the contour.
...
def update_image_segmentation(data_dir, mask_dir, image_name, mask_name, save_dir=None, save_vis=True):
    ...
    upper_body = body_mask - seg_mask# Point to be noted
    upper_body[upper_body > 0] = 20# Mark the extracted region as 20 (neck)
    ...
    upper_body[height//2:, :] = 0 # Pixels in lower half of the matrix defined 0
    # noise reduction
    ...
    if len(contours) > 0:
        ...
        x, y, w, h = cv2.boundingRect(c_neck)# Rectangular boundary of shape
        # draw the book contour (in green)
        cv2.rectangle(upper_body_vis, (x, y), (x + w, y + h), (170, 230, 0), 2)# Doubtful

    # make neck region mask
    neck_mask = np.zeros((fine_height, fine_width)).astype(np.int)
    for each in neck:
        neck_mask[each[0]][each[1]] = 20# each[0] = row_num and each[1] = col_num
